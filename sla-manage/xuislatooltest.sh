...
try:
    ...
    if not updated_users_log:
        ...
        for u in updated_users_log:
            ...
            # --- Traffic Column Formatting (BUG FIXED HERE) ---
            # We explicitly cast floats to string just in case
            tf_raw = f"{u['o_tf']}→{u['n_tf']}"
            
            # Padding calculation with explicit int()
            pad_len_tf = int(W_TRAF - 3 - len(tf_raw))
            if pad_len_tf < 0: pad_len_tf = 0
            
            # Force str() on float values to prevent concatenation error
            tf_str = f" {C.RED}{str(u['o_tf'])}{C.NC}→{tf_color}{str(u['n_tf'])}{C.NC}" + (" " * pad_len_tf)
